class Solution:
    def processQueries(self, queries: List[int], m: int) -> List[int]:
        ...
        answer = []
        for q in queries:
            pos = permutations.index(q)
            answer.append(pos)
            del permutations[pos]
            permutations.insert(0, q)
...
# We also use a fenwick tree. 
# This data structure is kind of like a virtual array A that lets us do two things in O(n log n) time: 
# .add(i, x) is like A[i] += x, and .sum(i) is like sum(A[:i+1]).
